...
__constant__ float F[FILTER_DIM][FILTER_DIM]
                    [FILTER_DIM]; // 3D kernel in constant memory

__global__ void conv3d_kernel(float *N, float *P, int radius, int width,
...
int main() {
  ...
  conv3d_kernel<<<gridDim, blockDim>>>(d_input, d_output, radius, width, height,
                                       depth);
