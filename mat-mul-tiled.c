/* Tiled matrix multiplication. The code assumes the height and width of the matrices are 
 * multiples of TILE_WIDTH which is also the dimension of the block in both the x and y 
 * direction
 */
...
__global__
void mat_mul_kernel(float* M, float* N, float* P, int width) {
...
torch::Tensor mat_mul_tiled(torch::Tensor M, torch::Tensor N) {
    ...
    mat_mul_kernel<<<dimGrid, dimBlock>>>(M.data_ptr<float>(), N.data_ptr<float>(), output.data_ptr<float>(), w_m);
